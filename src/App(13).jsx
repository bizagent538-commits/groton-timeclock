...
export default function App() {
  ...
  const exportCommitteeReport = (committeeId) => {
    const committee = committees.find(c => Number(c.id) === Number(committeeId));
    if (!committee) return;

    const committeeEntries = timeEntries.filter(e => Number(e.committee_id) === Number(committeeId) && e.clock_out);
  ...
  const exportWeeklyReports = () => {
    ...
    committees.forEach(committee => {
      const committeeEntries = timeEntries.filter(e => {
        ...
        return Number(e.committee_id) === Number(committee.id) && e.clock_out &&
  ...
  const exportReportsByDateRange = (startDate, endDate) => {
    // Parse as local dates to avoid timezone shift (YYYY-MM-DD parsed as UTC by default)
